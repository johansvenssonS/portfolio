...
const ProjectPage = () => {
  ...
  // Find project in either language by slug
  const projectSVIndex = projectsSV.findIndex(
    (p) => getProjectSlug(p.name) === projectSlug
  );
  const projectENIndex = projectsEN.findIndex(
    (p) => getProjectSlug(p.name) === projectSlug
  );

  // Get the index from whichever language matched
  const projectIndex = projectSVIndex !== -1 ? projectSVIndex : projectENIndex;

  // Get the project in the current language using the index
  const displayProject =
    projectIndex !== -1
      ? language === "en"
        ? projectsEN[projectIndex]
        : projectsSV[projectIndex]
      : undefined;
